...
function YourComponent() {
  ...
  const displayUsers = rideragents && rideragents.length > 0
  ? rideragents.map(createCard)
  : null;

  const fetchData = async (page) => {
    try {
      const response = await axios.get(`http://localhost:4000/rideragentsapi?page=${page + 1}&pageSize=${usersPerPage}`);
      console.log(response);
      const { rideragents: newrideragents, totalPages: newTotalPages, userId: userId } = response.data;
      setUserId(userId);
  ...
  async function rateAgentPositive(agentId, goodRating) {
    ...
    try {
      ...
      const response = await axios.post(`http://localhost:4000/patchratingrider?agentId=${agentId}&goodRating=${goodRating}`);
  ...
  async function rateAgentNegative(agentId, badRating) {
    ...
    try {
      // Send the vote to the server
      const response = await axios.post(`http://localhost:4000/patchratingrider?agentId=${agentId}&badRating=${badRating}`);
            ...
            <ul className='roommate-list-head'>
              <li>
                <div className='profilePic'>
                  ...
                  <p className='profile-body'>I am a a keke/okada rider around ifite axis </p>
                </div>
              </li>
