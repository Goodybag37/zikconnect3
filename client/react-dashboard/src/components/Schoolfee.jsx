...
function YourComponent() {
  ...
  const [schoolfeegReviews, setschoolfeegReviews] = useState();
  const [schoolfeebReviews, setschoolfeebReviews] = useState();
  ...
  const fetchData = async (page) => {
    try {
      ...
      const {
        schoolfeeagents: newschoolfeeagents,
        totalPages: newTotalPages,
        userId: userId,
        schoolfeeGreviews: schoolfeeGreviews,
        schoolfeeBreviews: schoolfeeBreviews,
      } = response.data;
      setUserId(userId);
      setschoolfeegReviews(schoolfeeGreviews);
      setschoolfeebReviews(schoolfeeBreviews);
  ...
  const handleShowProfile = async () => {
    try {
      ...
      if (result.status === 200 && result.data.length > 0) {
        ...
        setModalContent5(
          <div>
            <p>Oops, you are not a schoolfee Agent</p>
            <button
              style={{
                marginTop: "20px",
                background: "#ff0000",
  ...
  async function rateAgentPositive(agentId, goodRating) {
    ...
    try {
      const response = await axios.post(
        `${apiUrl}/api/patchratingschoolfee?agentId=${agentId}&goodRating=${goodRating}`
      );
  ...
  const handleConnectClick = async (agentId) => {
    ...
    const selectedAgent = schoolfeeagents.find((agent) => agent.id === agentId);
  ...
  async function rateAgentNegative(agentId, badRating) {
    ...
    try {
      const response = await axios.post(
        `${apiUrl}/api/patchratingschoolfee?agentId=${agentId}&badRating=${badRating}`
      );
                  ...
                  <li>
                    <div className="profilePic">
                      ...
                      <p className="profile-body">
                        Pay your schoolfees here and other payments
                      </p>
                    </div>
                  </li>
  ...
  const displayUsers =
    schoolfeeagents && schoolfeeagents.length > 0
      ? schoolfeeagents.map((agent) =>
