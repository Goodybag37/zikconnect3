...
function YourComponent() {
  const [cryptoagents, setCryptoagents] = useState([]);
  ...
  const [cryptogReviews, setcryptogReviews] = useState();
  const [cryptobReviews, setcryptobReviews] = useState();
  ...
  const fetchData = async (page) => {
    try {
      ...
      const {
        ...
        userId: userId,
        cryptoGreviews: cryptoGreviews,
        cryptoBreviews: cryptoBreviews,
      } = response.data;
      setUserId(userId);
      setcryptogReviews(cryptoGreviews);
      setcryptobReviews(cryptoBreviews);
      setCryptoagents(newcryptoagents);
      setTotalPages(newTotalPages);
      if (newcryptoagents.length > 0) {
        setGoodRating(parseInt(newcryptoagents[0].good_rating, 10));
      }
    } catch (error) {
      console.error(error);
    }
  ...
  const handleShowProfile = async () => {
    try {
      ...
      if (result.status === 200 && result.data.length > 0) {
        ...
        setModalContent5(
          <div>
            <p>Oops, you are not a Crypto Agent</p>
            <button
              style={{
                marginTop: "20px",
                background: "#ff0000",
  ...
  const handleConnectClick = async (agentId) => {
    ...
    const selectedAgent = cryptoagents.find((agent) => agent.id === agentId);
                  ...
                  <li>
                    <div className="profilePic">
                      ...
                      <p className="profile-body">
                        I sell all forms of cryptocurrencies ranging from
                        bitcoin, etherium, and USDT
                      </p>
                    </div>
                  </li>
  ...
  const displayUsers =
    cryptoagents && cryptoagents.length > 0
