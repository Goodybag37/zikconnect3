...
function YourComponent() {
  ...
  const [deliverygReviews, setdeliverygReviews] = useState();
  const [deliverybReviews, setdeliverybReviews] = useState();
  ...
  const fetchData = async (page) => {
    try {
      const response = await axios.get(
        `${apiUrl}/api/deliveryagentsapi?page=${
          page + 1
        }&pageSize=${usersPerPage}`
      );
      console.log(response);
      const {
        deliveryagents: newdeliveryagents,
        totalPages: newTotalPages,
        userId: userId,
        deliveryGreviews: deliveryGreviews,
        deliveryBreviews: deliveryBreviews,
      } = response.data;
      setUserId(userId);
      setdeliverygReviews(deliveryGreviews);
      setdeliverybReviews(deliveryBreviews);
      setdeliveryagents(newdeliveryagents);
      setTotalPages(newTotalPages);
      if (newdeliveryagents.length > 0) {
        setGoodRating(parseInt(newdeliveryagents[0].good_rating, 10));
      }
    } catch (error) {
      console.error(error);
    }
  ...
  const handleShowProfile = async () => {
    try {
      const result = await axios.get(
        `${apiUrl}/api/agentprofile?userId=${userbread}&type=deliveryagents`,
        { headers: { "Content-Type": "application/x-www-form-urlencoded" } }
      );
      ...
      if (result.status === 200 && result.data.length > 0) {
        ...
        setModalContent5(
          <div>
            <p>Oops, you are not a delivery Agent</p>
            <button
              style={{
                marginTop: "20px",
                background: "#ff0000",
  ...
  async function rateAgentPositive(agentId, goodRating) {
    ...
    try {
      const response = await axios.post(
        `${apiUrl}/api/patchratingdelivery?agentId=${agentId}&goodRating=${goodRating}`
      );
  ...
  async function rateAgentNegative(agentId, badRating) {
    ...
    try {
      const response = await axios.post(
        `${apiUrl}/api/patchratingdelivery?agentId=${agentId}&badRating=${badRating}`
      );
                  ...
                  <li>
                    <div className="profilePic">
                      ...
                      <p className="profile-body">
                        I run deliveries for everything you wish to buy
                      </p>
                    </div>
                  </li>
