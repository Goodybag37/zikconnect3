...
function YourComponent() {
  ...
  const displayUsers = deliveryagents && deliveryagents.length > 0
  ? deliveryagents.map(createCard)
  : null;

  const fetchData = async (page) => {
    try {
      const response = await axios.get(`http://localhost:4000/deliveryagentsapi?page=${page + 1}&pageSize=${usersPerPage}`);
      console.log(response);
      const { deliveryagents: newdeliveryagents, totalPages: newTotalPages, userId: userId } = response.data;
      setUserId(userId);
  ...
  async function rateAgentPositive(agentId, goodRating) {
    ...
    try {
      ...
      const response = await axios.post(`http://localhost:4000/patchratingdelivery?agentId=${agentId}&goodRating=${goodRating}`);
  ...
  async function rateAgentNegative(agentId, badRating) {
    ...
    try {
      // Send the vote to the server
      const response = await axios.post(`http://localhost:4000/patchratingdelivery?agentId=${agentId}&badRating=${badRating}`);
            ...
            <ul className='roommate-list-head'>
              <li>
                <div className='profilePic'>
                  ...
                  <p className='profile-body'>I deliver packages to any where around ifite and tempsite </p>
                </div>
              </li>
