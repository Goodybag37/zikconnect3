...
function YourComponent() {
  ...
  const fetchData = async (page) => {
    try {
      const response = await axios.get(
        `${apiUrls}/api/deliveryagentsapi?page=${
          page + 1
      ...
      if (newdeliveryagents.length > 0) {
        const agentRatings = {};
        newdeliveryagents.forEach((agent) => {
  ...
  const handleShowProfile = async () => {
    try {
      const result = await axios.get(
        `${apiUrls}/api/agentprofile?userId=${userbread}&type=deliveryagents`,
        { headers: { "Content-Type": "application/x-www-form-urlencoded" } }
      );
  ...
  async function rateAgentPositive(agentId) {
    ...
    const agentType = "deliveryagents";
