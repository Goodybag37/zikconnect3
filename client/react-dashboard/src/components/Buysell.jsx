...
function YourComponent() {
  ...
  const fetchData = async (page, searchQuery) => {
    try {
      const response = await axios.get(`${apiUrls}/api/buysellapi`, {
        params: {
          page: page + 1,
  ...
  const handleShowPicture = async (itemId) => {
    try {
      const response = await axios.get(`${apiUrls}/api/buysell/${itemId}`, {
                ...
                <li className="roommate-list">
                  ...
                  <button
                    onClick={() => {
                      ...
                      handleShowPicture(buysell.id);
                    }}
                    className="roommateButtonPicture "
                  >
                    <BsPeopleFill className="connect_icon" />
                    {loadingP[buysell.id] ? "Loading..." : "See Picture"}
                  </button>
                  {selectedAgent && (
                    <BsArchiveFill
