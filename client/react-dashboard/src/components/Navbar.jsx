  ...
  return (
    <div>
     <nav className=' w-full flex py-6 justify-between items-center navbar'>
       <img src={logo} alt='zikconnect' className='w-[124px] h-[32px]'/> 
     
       <ul className='list-none z-[5] sm:flex hidden justify-end items-center flex-1'>
      {navLinks.map((nav, index) => (
        <li key={nav.id}
        className={`font-poppins font-normal cursor-pointer  
        text-[16px] ${index === navLinks.length - 1 ? 'mr-20' : 'mr-10'} text-white`}>
