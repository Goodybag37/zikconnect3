  ...
  const fetchData = async (page) => {
    try {
      const response = await axios.get(
        `http://localhost:4000/api/roommatesapi?page=${
          page + 1
        }&pageSize=${usersPerPage}`
      );
      console.log("Response data:", response.data);
      const { roommates: newRoommates, totalPages: newTotalPages } =
        response.data;
        ...
        <div className="card-inner">
          <div key={roommate.id}>
            <ul className="roommate-list-head">
              ...
              <li className="roommate-list">Name: {roommate.fullname}</li>
              <li className="roommate-list">Dept: {roommate.department}</li>
              <li className="roommate-list">Gender: {roommate.gender}</li>
              <li className="roommate-list">Phone: {roommate.phone}</li>
  ...
  return (
    <main className="main-container">
      <div className="main-title">
        <form
          className="input-group box-shadow"
          action="/department"
          method="get"
        >
