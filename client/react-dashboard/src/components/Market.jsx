...
function YourComponent() {
  ...
  const fetchData = async (page, searchQuery) => {
    try {
      const response = await axios.get(`${apiUrls}/api/marketapi`, {
        params: {
          page: page + 1,
      ...
      const { markets: newMarkets, totalPages: newTotalPages } = response.data;
  ...
  const fetchSettingStatus = async (userId) => {
    try {
      ...
      if (settings && settings.toggle_status && settings.toggle_status.market) {
        const toggleStatus = settings.toggle_status.market;
  ...
  const handleConnectClick = async (itemId) => {
    ...
    const processConnection = async (
      ...
      const response = await axios.get(`${apiUrls}/api/market-face/${itemId}`, {
      ...
      const selectedAgent = markets.find((agent) => agent.id === itemId);
      // setSelectedAgent(selectedAgent);
      const selected = selectedAgent.contact;

      console.log("the url of image is", imageUrl);
      ...
      const content = (
        <>
          <div className="locationInfo">
            ...
            <p className="popup-paragraph">
              You have placed an order with this brand. the details of your
            ...
            {selectedAgent && (
              <>
                ...
                <p className="locationInfo">
                  Owners Full Name: {selectedAgent.seller_name}
                </p>
                <p className="locationInfo">
                  Owners USER ID: {selectedAgent.fk_user_id}
                </p>
                <p className="locationInfo">
  ...
  const handleShowPicture = async (itemId) => {
    try {
      const response = await axios.get(`${apiUrls}/api/market/${itemId}`, {
        responseType: "blob", // Important: Fetch the image as a Blob
        validateStatus: (status) => status < 500,
      });
  ...
  const handleDeleteItem = async () => {
    ...
    setmarkets((prevMarkets) =>
      prevMarkets.filter((market) => market.id !== itemToDelete)
    );
  ...
  function createCard(market) {
    const isFlipped = flippedCards[market.id];
            ...
            <form encType="multipart/form-data" onSubmit={handleEditSubmit}>
              ...
              <div className="inputGroup2 pictureError">
                <input
                  ...
                  type="file"
                  id="file"
                  placeholder={market.original_name}
                  onClick={() => {
                    setErrorMessage("");
                    setError("");
                  }}
            ...
            <div key={market.id}>
              <ul className="roommate-list-head">
                  ...
                  <div className="profilePicRoommate">
                    <div className="profileHeaderR bg-blue-gradient">
                      <p className="profileInfo">
                        {market.status == "order" ? "In order.." : market.name}
                      </p>
                    </div>
                ...
                <li className="roommateList">
                  At :{" "}
                  {market.status == "order" ? "In order.." : market.location}{" "}
                </li>
                <hr />
                <li className="roommateList">
                  On :{" "}
                  {market.status == "order"
                    ? "In order.."
                    : market.formatted_date}
                </li>
                <hr />
                <li className="roommateList">
                  By :{" "}
                  {market.status == "order" ? "In order.." : market.seller_name}
                </li>
                <li className="roommate-list">
                  <button
                    className="bg-blue-gradient roommateButtonConnect"
                    disabled={
                      buttonStatus[market.status] === "order" ||
                      // selectedAgent ||
                      market.fk_user_id === userIn
                    }
                    onClick={() => handleConnectClick(market.id)}
                  >
                  ...
                  {selectedAgent && (
                    <BsEyedropper
                      ...
                      onClick={() => {
                        handleEditClick(market);
                        handleFlip(market.id);
                      }}
                    />
                  )}
                </li>
  ...
  const filteredMarkets = markets.filter((market) => {
    if (viewMode === "general") {
      return market.status === "available" || market.status === "order";
    }
    return true; // Show all items in profile view
  });

  const displayUsers = filteredMarkets
    .filter((market) =>
      selectedAgent ? market.fk_user_id === selectedAgent : true
    )
    .map(createCard);

  const displayUsers2 = filteredMarkets.map(createCard);
