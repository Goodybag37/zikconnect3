...
function YourComponent() {
  ...
  const displayUsers = cybercafeagents && cybercafeagents.length > 0
  ? cybercafeagents.map(createCard)
  : null;

  const fetchData = async (page) => {
    try {
      const response = await axios.get(`http://localhost:4000/cybercafeagentsapi?page=${page + 1}&pageSize=${usersPerPage}`);
      console.log(response);
      const { cybercafeagents: newcybercafeagents, totalPages: newTotalPages, userId: userId } = response.data;
      setUserId(userId);
  ...
  async function rateAgentPositive(agentId, goodRating) {
    ...
    try {
      ...
      const response = await axios.post(`http://localhost:4000/patchratingcybercafe?agentId=${agentId}&goodRating=${goodRating}`);
  ...
  async function rateAgentNegative(agentId, badRating) {
    ...
    try {
      // Send the vote to the server
      const response = await axios.post(`http://localhost:4000/patchratingcybercafe?agentId=${agentId}&badRating=${badRating}`);
            ...
            <ul className='roommate-list-head'>
              <li>
                <div className='profilePic'>
                  ...
                  <p className='profile-body'>i do all printing and online registerations located at bamboo</p>
                </div>
              </li>
