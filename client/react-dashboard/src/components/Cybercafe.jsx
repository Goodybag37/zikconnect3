...
function YourComponent() {
  const [cybercafeagents, setcybercafeagents] = useState([]);
  ...
  const [cybercafegReviews, setcybercafegReviews] = useState();
  const [cybercafebReviews, setcybercafebReviews] = useState();
  ...
  const fetchData = async (page) => {
    try {
      const response = await axios.get(
        `${apiUrl}/api/cybercafeagentsapi?page=${
          page + 1
        }&pageSize=${usersPerPage}`
      );
      console.log(response);
      const {
        cybercafeagents: newcybercafeagents,
        totalPages: newTotalPages,
        userId: userId,
        cybercafeGreviews: cybercafeGreviews,
        cybercafeBreviews: cybercafeBreviews,
      } = response.data;
      setUserId(userId);
      setcybercafegReviews(cybercafeGreviews);
      setcybercafebReviews(cybercafeBreviews);
      setcybercafeagents(newcybercafeagents);
      setTotalPages(newTotalPages);
      if (newcybercafeagents.length > 0) {
        setGoodRating(parseInt(newcybercafeagents[0].good_rating, 10));
      }
    } catch (error) {
      console.error(error);
    }
  ...
  const handleShowProfile = async () => {
    try {
      const result = await axios.get(
        `${apiUrl}/api/agentprofile?userId=${userbread}&type=cybercafeagents`,
        { headers: { "Content-Type": "application/x-www-form-urlencoded" } }
      );
      ...
      if (result.status === 200 && result.data.length > 0) {
        ...
        setModalContent5(
          <div>
            <p>Oops, you are not a cybercafe Agent</p>
            <button
              style={{
                marginTop: "20px",
                background: "#ff0000",
  ...
  async function rateAgentPositive(agentId, goodRating) {
    ...
    try {
      const response = await axios.post(
        `${apiUrl}/api/patchratingcybercafe?agentId=${agentId}&goodRating=${goodRating}`
      );
  ...
  const handleConnectClick = async (agentId) => {
    ...
    const selectedAgent = cybercafeagents.find((agent) => agent.id === agentId);
  ...
  async function rateAgentNegative(agentId, badRating) {
    ...
    try {
      const response = await axios.post(
        `${apiUrl}/api/patchratingcybercafe?agentId=${agentId}&badRating=${badRating}`
      );
                  ...
                  <li>
                    <div className="profilePic">
                      ...
                      <p className="profile-body">
                        Print your project work here and run all your printing
                        jobs here
                      </p>
                    </div>
                  </li>
  ...
  const displayUsers =
    cybercafeagents && cybercafeagents.length > 0
      ? cybercafeagents.map((agent) =>
