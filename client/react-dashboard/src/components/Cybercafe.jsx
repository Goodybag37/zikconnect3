...
function YourComponent() {
  ...
  const fetchData = async (page) => {
    try {
      const response = await axios.get(
        `${apiUrls}/api/cybercafeagentsapi?page=${
          page + 1
      ...
      if (newcybercafeagents.length > 0) {
        const agentRatings = {};
        newcybercafeagents.forEach((agent) => {
  ...
  const handleShowProfile = async () => {
    try {
      const result = await axios.get(
        `${apiUrls}/api/agentprofile?userId=${userbread}&type=cybercafeagents`,
        { headers: { "Content-Type": "application/x-www-form-urlencoded" } }
      );
  ...
  async function rateAgentPositive(agentId) {
    ...
    const agentType = "cybercafeagents";
