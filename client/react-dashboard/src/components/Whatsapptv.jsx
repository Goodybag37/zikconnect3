...
function YourComponent() {
  ...
  const fetchData = async (page) => {
    try {
      const response = await axios.get(
        `${apiUrls}/api/whatsapptvagentsapi?page=${
          page + 1
      ...
      if (newwhatsapptvagents.length > 0) {
        const agentRatings = {};
        newwhatsapptvagents.forEach((agent) => {
  ...
  const handleShowProfile = async () => {
    try {
      const result = await axios.get(
        `${apiUrls}/api/agentprofile?userId=${userbread}&type=whatsapptvagents`,
        { headers: { "Content-Type": "application/x-www-form-urlencoded" } }
      );
  ...
  async function rateAgentPositive(agentId) {
    ...
    const agentType = "whatsapptvagents";
