...
function YourComponent() {
  ...
  const displayUsers = whatsapptvagents && whatsapptvagents.length > 0
  ? whatsapptvagents.map(createCard)
  : null;

  const fetchData = async (page) => {
    try {
      const response = await axios.get(`http://localhost:4000/whatsapptvagentsapi?page=${page + 1}&pageSize=${usersPerPage}`);
      console.log(response);
      const { whatsapptvagents: newwhatsapptvagents, totalPages: newTotalPages, userId: userId } = response.data;
      setUserId(userId);
  ...
  async function rateAgentPositive(agentId, goodRating) {
    ...
    try {
      ...
      const response = await axios.post(`http://localhost:4000/patchratingwhatsapptv?agentId=${agentId}&goodRating=${goodRating}`);
  ...
  async function rateAgentNegative(agentId, badRating) {
    ...
    try {
      // Send the vote to the server
      const response = await axios.post(`http://localhost:4000/patchratingwhatsapptv?agentId=${agentId}&badRating=${badRating}`);
            ...
            <ul className='roommate-list-head'>
              <li>
                <div className='profilePic'>
                  ...
                  <p className='profile-body'>i sell all forms of whatsapptvcurencies ranging from bitcoin etherium and usdt</p>
                </div>
              </li>
