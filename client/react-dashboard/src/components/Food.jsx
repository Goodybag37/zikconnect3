...
function YourComponent() {
  const [foodagents, setFoodagents] = useState([]);
  ...
  const [foodgReviews, setFoodgReviews] = useState();
  const [foodbReviews, setFoodbReviews] = useState();
  ...
  const fetchData = async (page) => {
    try {
      const response = await axios.get(
        `${apiUrls}/api/foodagentsapi?page=${page + 1}&pageSize=${usersPerPage}`
      );

      const {
        foodagents: newfoodagents,
        totalPages: newTotalPages,
        userId: userId,
        foodGreviews: foodGreviews,
        foodBreviews: foodBreviews,
      } = response.data;
      setUserId(userId);
      setFoodgReviews(foodGreviews);
      setFoodbReviews(foodBreviews);
      setFoodagents(newfoodagents);
      setTotalPages(newTotalPages);
      if (newfoodagents.length > 0) {
        const agentRatings = {};
        newfoodagents.forEach((agent) => {
  ...
  const handleShowProfile = async () => {
    try {
      const result = await axios.get(
        `${apiUrls}/api/agentprofile?userId=${userbread}&type=foodagents`,
        { headers: { "Content-Type": "application/x-www-form-urlencoded" } }
      );
      ...
      if (result.status === 200 && result.data.length > 0) {
        ...
        setModalContent5(
          <div>
            <p>Oops, you are not a Food Agent</p>
            <button
              style={{
                marginTop: "20px",
                background: "#ff0000",
  ...
  const processConnection = async (agentId, latitude, longitude, locationM) => {
    ...
    const selectedAgent = foodagents.find((agent) => agent.id === agentId);
  ...
  const displayUsers =
    foodagents && foodagents.length > 0
      ? foodagents.map((agent) =>
